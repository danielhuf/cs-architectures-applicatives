...
function update(messages) {
	...
	messagesList.reverse().map(formatMsg).forEach(function(message, index) {
		...
		deleteBtn.setAttribute('data-message-index', index);
		deleteBtn.addEventListener('click', function() {
			const messageIndex = this.getAttribute('data-message-index');
			const actualIndex = messagesList.length - 1 - messageIndex;
			fetch(msgsUrl + "msg/del/" + actualIndex)
